...
| 開発言語 | 選定理由 | 推奨ライブラリ例 |
| :--- | :--- | :--- |
| C# | 静的型付け、豊富なライブラリ、Windowsとの親和性が高い。 | Geometry3Sharp, Clipper2, .NET Libraries |
...
| **計算エンジン<br>(バックエンド)** | **C# (.NET 8)** | ツールパス計算などの処理を担当します。静的型付言語による堅牢性と高速な実行速度を活用します。 |
...
*   **関心の分離:** UI（Electron/React）と計算ロジック（C#）が明確に分離されるため、それぞれの開発に集中でき、メンテナンス性が向上します。
...
4.  UIからの操作（例: 「ツールパス生成」ボタンのクリック）をトリガーに、Electronの機能を使って**C#**の計算処理を呼び出す。
5.  C#が生成したツールパスのデータをReactに渡し、**Three.js**で3Dビューア上に描画する。
...
| **手動操作 (Jog) & 原点設定** | ・X, Y, Z軸の各方向（+, -）への手動移動。<br>・移動量（例: 0.01mm〜100mm）の選択機能。<br>・現在の工具位置を加工原点（ワーク座標系 G54）として設定する機能（`G10 L20 P1 X0 Y0 Z0`）。<br>・機械原点（マシン座標系）をリセット/ホーミング（`$H`）するボタン機能（誤操作防止のため1.5秒間の長押しが必要。マシン設定で有効化されている場合のみ利用可能）。 |
| **状態表示とコンソール** | ・マシンの状態（例: Idle, Run, Alarm）のリアルタイム表示。<br>・加工座標（WPos）および機械座標（MPos）のリアルタイム表示。<br>・マシンとの間で送受信されるすべてのコマンドと応答をリアルタイムで表示するコンソール機能。 |
| **加工機パラメータ設定** | ・Grblパラメータ（各軸のステップ数/mm、軸方向反転マスク等）の読み込み・表示機能。<br>・UI上で各軸のパルス送り量（ステップ/mm）や正転逆転（軸の反転）を設定し、Grblへ書き込む（保存する）機能。<br>・機械原点のリセット機能を有効化/無効化する安全設定（「機械原点のリセットを有効にする」）。 |

## 10. セットアップと起動

### 10.1 前提条件
- Node.js (v18以上推奨)
- .NET 8 SDK (WPF開発・実行環境)
- Microsoft Edge WebView2 ランタイム (フロントエンドの描画に必要)

### 10.2 セットアップ
プロジェクトルートで以下を実行し、フロントエンドの依存パッケージをインストールします。
...
#### 開発モード (Development)
フロントエンドの開発サーバーを起動した状態で、C# バックエンドを実行します。これにより、フロントエンド側の変更が即座に反映されます。
...
2. **C# バックエンドの起動**
   別のターミナルを開き、C# プロジェクトディレクトリに移動して実行します。
   ```bash
   cd src/csharp/GeoCraft.Desktop
   dotnet run
   ```

*※ プロジェクトルートにある `start_dev.bat` を実行することでも、開発サーバーの起動とバックエンドの立ち上げを自動で行うことができます。*

#### 本番モード (Production)
フロントエンドをビルドして C# アプリに組み込み、スタンドアロンの実行ファイルを作成・起動します。

1. **一括ビルドスクリプトの実行**
   プロジェクトルートにある `build.bat` をダブルクリックして実行するか、手動でビルドします。

2. **アプリの起動**
   ビルド成果物である実行ファイルを起動します。
   ```bash
   cd bin\Release\net8.0-windows\publish
   GeoCraft.Desktop.exe
   ```
   *※ プロジェクトルートにある `start_prod.bat` を実行することでも、本番用バイナリから起動できます。*